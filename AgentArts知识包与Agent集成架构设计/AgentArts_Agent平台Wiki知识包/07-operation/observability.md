...
> **官方材料基线**：0916《观测与优化智能体》**第 1 章「智能体观测」**（1–116 页）+ 第 2 章「智能体评估」+ 第 3 章「智能体优化」。分册名从 0804 的《智能体运营运维》改为《观测与优化智能体》，内容从 2 章扩为 3 章。
>
> **术语变更（务必用新词）**：`高代码智能体数据` → **智能体运行时数据**；`高代码应用运行数据` → **托管智能体数据**；`沙箱工具数据信息` → **代码解释器数据信息**；`上报第三方智能体 Trace、Metric、Log 数据` → **上报第三方智能体观测数据**。
...
- **优化**：0916 已提供**独立的「观测与优化 > 优化」页签**，包含四类平台化任务——**轨迹分析**（诊断归因）、**工具优化**、**Skill 优化**（配置层，即时生效）、**模型优化**（模型层，GRPO 在线强化学习）。详见 [智能体优化](optimization.md)。

> ⚠️ **旧结论已失效**：0804 材料《智能体运营运维》没有优化章节（只有观测 + 评估两章），当时"优化"只能理解为跨观测/回流/评估的工作流；**0916 已把它产品化为独立页签与任务**。
...
| 查看托管智能体数据 | 智能体运行时、**代码解释器**、网关日志 | 托管资源故障排查（0916 菜单：观测与优化 > 观测） |
...
| 本地高代码并托管到 AgentArts Runtime | **上报智能体运行时数据** | 开启运行时/代码解释器/网关日志后，在「观测与优化 > 观测 > 查看托管智能体数据」查看 |
| Managed Agents | 平台原生（可观测配置开关） | 创建智能体时「可观测配置」控制日志是否上报 LTS，**默认已开启**；未开启则运行日志无法上报 LTS |
| 非 AgentArts 开发/部署的第三方智能体 | **上报第三方智能体观测数据**（观测 OpenAPI） | Trace/Metric 遵循 OpenTelemetry，Log 调用 LTS REST API |

### 托管智能体上报边界

托管智能体当前明确开放的是 **运行时、代码解释器、网关日志**。**托管智能体的调用链与指标查看功能仍在规划中，暂未开放。** 因此不应因 SDK 配置中存在 `tracing` / `metrics` 字段就宣称当前控制台已支持托管 Trace/Metric 可视化。

日志开启与查看（0916 菜单路径）：

- 运行时：「托管与运行 > 运行时 > 详情 > 日志记录」。
- 代码解释器：「托管与运行 > 代码解释器 > 创建/详情 > 日志记录」。
- 浏览器：「托管与运行 > 浏览器 > 创建/详情 > 日志记录」。
- 网关：「托管与运行 > 网关 > 创建网关 > 日志记录」。
- 聚合验证：触发一次调用后，进入「**观测与优化 > 观测 > 查看托管智能体数据**」，分别查看「智能体运行时」、「**代码解释器**」和「网关」页签。
- **第三方 Skill 观测数据上报**也在 0916 提供（《最佳实践》3.1.1）。
- SDK 会在所有控制面/数据面请求的 `User-Agent` 末尾追加 `os/<system>/<release> agentarts-sdk-python/<version>`，便于在服务端日志中定位调用方版本与操作系统。
...
**0916 新增：SDK 接入支持两种框架**

| 框架 | 章节 |
| --- | --- |
| LangChain / LangGraph | 观测与优化智能体 1.2.4.4.1 |
| **AgentScope**（0916 新增） | 观测与优化智能体 1.2.4.4.2 |
